...
import { ChevronDown, Eye, Wrench, LayoutDashboard, LineChart, Database, Menu, X } from "lucide-react";
...
function ActiveLink({
  ...
  return (
    <Link
      ...
      className={cn(
        mobile ? "block w-full py-3" : "text-xs md:text-md",
        "font-semibold uppercase transition-colors",
        isActive ? "text-[#f3a84f]" : "text-[#555555] dark:text-white hover:text-[#f3a84f] dark:hover:text-[#f3a84f]",
        className
      )}
    >
      {children}
...
function MegaItem({
  ...
  return (
    <Link href={href} className="group flex items-start gap-3 rounded-lg p-2 -m-2 hover:bg-white/40 transition">
      <div className="mt-1 text-[#555555] dark:text-white">{icon}</div>
      <div>
        <div className="flex items-center justify-between">
          <h4 className="font-semibold text-[#555555] dark:text-white">{title}</h4>
        </div>
      ...
      >
        <div className="flex items-center justify-between">
          <Link href="/" className="inline-flex items-center">
            <Image src="/mazo-logo.png" alt="Logo" width={160} height={44} className="h-11 w-auto" />
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-4  md:gap-5 lg:gap-8">
            ...
            <button
              ...
              className="flex items-center gap-1 font-semibold text-xs md:text-md  uppercase dark:text-white text-[#555555] hover:text-[#f3a84f] dark:hover:text-[#f3a84f] transition"
            >
              SERVICES
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-6 md:p-8">
              ...
              <div className="md:col-span-5">
                <div className="rounded-xl bg-[#f3a84f] text-white p-6 md:p-7">
                  <h3 className="text-2xl font-semibold mb-2">Mode is the intelligence layer for your data stack</h3>
                  <div className="mt-6 space-y-3">
                    <Link href="/platform" className="group inline-flex items-center gap-2 font-semibold">
                      Platform overview <span className="transition group-hover:translate-x-0.5">›</span>
                    </Link>
                    <Link href="/tour" className="group inline-flex items-center gap-2 font-semibold">
                      Product tour <span className="transition group-hover:translate-x-0.5">›</span>
                    </Link>
                  </div>
                </div>
              </div>
              ...
              <div className="md:col-span-7">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                  <MegaItem icon={<Wrench className="h-5 w-5" />} title="Ad Hoc Analysis" desc="Standalone reports for key questions" href="/services/ad-hoc" />
                  <MegaItem icon={<Eye className="h-5 w-5" />} title="Advanced Analytics" desc="Model & analyze on one platform" href="/services/advanced-analytics" />
                  <MegaItem icon={<LayoutDashboard className="h-5 w-5" />} title="Self-Serve Reporting" desc="Governed datasets & metrics" href="/services/self-serve" />
                  <MegaItem icon={<LineChart className="h-5 w-5" />} title="Interactive Dashboards" desc="Drag-and-drop visual tools" href="/services/dashboards" />
                  <MegaItem icon={<Database className="h-5 w-5" />} title="Custom Data Apps" desc="Internal tools at scale" href="/services/data-apps" />
                </div>
              </div>
            </div>
          </div>
        ...
        >
          <div className="rounded-b-2xl  bg-white dark:bg-[#0f131b] shadow-lg overflow-hidden">
            <nav className="flex flex-col px-4 py-3">
              <ActiveLink mobile href="/" onClick={() => setMobileOpen(false)}>Home</ActiveLink>
              ...
              >
                <ul className="pl-3 pb-2 space-y-2">
                  <li><Link href="/services/ad-hoc" className="block py-2 dark:text-white" onClick={() => setMobileOpen(false)}>Ad Hoc Analysis</Link></li>
                  <li><Link href="/services/advanced-analytics" className="block py-2" onClick={() => setMobileOpen(false)}>Advanced Analytics</Link></li>
                  <li><Link href="/services/self-serve" className="block py-2" onClick={() => setMobileOpen(false)}>Self-Serve Reporting</Link></li>
                  <li><Link href="/services/dashboards" className="block py-2" onClick={() => setMobileOpen(false)}>Interactive Dashboards</Link></li>
                  <li><Link href="/services/data-apps" className="block py-2" onClick={() => setMobileOpen(false)}>Custom Data Apps</Link></li>
                </ul>
              </div>

              <ActiveLink mobile href="/aboutus" onClick={() => setMobileOpen(false)}>About Us</ActiveLink>
              <ActiveLink mobile href="/contactus" onClick={() => setMobileOpen(false)}>Contact Us</ActiveLink>
